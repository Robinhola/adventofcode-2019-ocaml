...
required is also 2.  For a mass of 1969, the fuel required is 654.  For a mass
of 100756, the fuel required is 33583.  The Fuel Counter-Upper needs to know the
...
module T : sig
  ...
  let name () = print_endline "--- Day 1: The Tyranny of the Rocket Equation ---"

  let compute_fuel mass = mass / 3 - 2

  let fuel_requirements = List.map lines ~f:compute_fuel

  let sum l = Option.value (List.reduce l ~f:(Int.( + ))) ~default:0
  ...
  let part2 () =
    let rec compute_all_fuel mass total = 
      match mass > 0 with 
      | true -> 
        let fuel = compute_fuel mass in
        compute_all_fuel fuel (total + fuel)
      | false -> total
    in
    let fuel_requirements = List.map lines ~f:(fun mass -> compute_all_fuel mass 0) in
    printf "part2:\t%i\n" (sum fuel_requirements)
  ;;
